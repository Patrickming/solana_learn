...
//处理上传图像和元数据的过程，并返回元数据 URI。
//该函数将以 Metaplex 实例和 NFT 数据作为输入，并返回元数据 URI 作为输出。
...
//辅助函数来处理创建 NFT。此函数以 Metaplex 实例、元数据 URI 和 NFT 数据作为输入。
//它使用 SDK 的 `create` 方法来创建 NFT，将元数据 URI、名称、卖方费用（seller fee）和标志作为参数传递。
//`createNft` 记录了铸币厂的 URL，并返回一个包含有关新创建的 NFT 的信息的 `nft` 对象。
//NFT 将铸造到与设置 Metaplex 实例时使用的 Identity Driver 相对应的公钥
  ...
  const metaplex = Metaplex.make(connection)
    ...
    .use(
      bundlrStorage({
        ...
        providerUrl: "https://api.devnet.solana.com",//这里不知道要不要改
        timeout: 60000,
      })
    )
